...
type Props = {
  ...
  hint?: string;
  /**
   * 연속 모드 - 한 권 읽고 멈추지 않고 계속 읽습니다(여러 권을 주르륵 찍을 때).
   * 같은 값이 연달아 읽히는 건 몇 초 동안 무시합니다.
   */
  continuous?: boolean;
};
 ...
 * 됩니다).
 */
export default function BarcodeScanner({ onDetect, accept, hint, continuous }: Props) {
  ...
  const handle = useCallback((raw: string) => {
    ...
    if (continuousRef.current) {
      // 같은 책이 계속 보이는 동안 여러 번 담기지 않도록 3초간 같은 값은 무시합니다.
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl bg-black">
      ...
      {continuous && count > 0 && (
        <span className="absolute top-3 right-3 rounded-full bg-emerald-500 px-3 py-1 text-sm font-bold text-white">
          {count}권 읽음
        </span>
      )}
